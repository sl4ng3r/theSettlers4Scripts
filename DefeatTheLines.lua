...
function sendSettlers(attackingKi, from, to, settlerType, amount)

    Settlers.AddSettlers(from.x, from.y, attackingKi.playerId, settlerType, amount)
    local warriors = WarriorsLib.SelectWarriors(from.x,from.y,10,attackingKi.playerId,settlerType)
    WarriorsLib.Send(warriors,to.x,to.y,WarriorsLib.MOVE_FORWARD);
...
function doActionsAfterMinutes()
    ...
    if newMinute() == 1 then
        if Game.Time() >= 1 then
            ...
            sendSettlers(KIs.kiRight, KIs.kiRight.top, KIs.kiLeft.top,Settlers.SWORDSMAN_01,5 )
            sendSettlers(KIs.kiRight, KIs.kiRight.mid, KIs.kiLeft.mid,Settlers.SWORDSMAN_01,5 )
            sendSettlers(KIs.kiRight, KIs.kiRight.bot, KIs.kiLeft.bot,Settlers.SWORDSMAN_01,5 )
